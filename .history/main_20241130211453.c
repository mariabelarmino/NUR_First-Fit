        ...
        if (!memoria[i].livre && memoria[i].processo != NULL) {
            ...
            if (chance_referenciado < 5) {
                if (processo->R == 1) {
                    ...
                    atualizarClasse(processo);
                    printf("%d     | processo %d| Processo %d foi escolhido pela aleatoriedade e modificou seu bit R para 0, indo para classe %d\n", tempo_atual, processo->id, processo->id, processo->classe);
                }
            }
            if (chance_modificado < 5) {
                if (processo->M == 1) {
                    ...
                    atualizarClasse(processo);
                    printf("%d     | processo %d| Processo %d foi escolhido pela aleatoriedade e modificou seu bit M para 0, indo para classe %d\n", tempo_atual, processo->id, processo->id, processo->classe);
                }
            }
        }
    }
...
int main() {
    ...
    while (1) {
        ...
        // Atualizacao dos bits R e M antes de tudo
