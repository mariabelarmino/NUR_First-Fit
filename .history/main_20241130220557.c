...
// Define constantes para o número máximo de processos e o intervalo de tempo para o ciclo de clock
...
typedef struct {
    ...
    int tamanho;                // Tamanho do processo na memória
    int tempo_chegada;          // Tempo em que o processo chega para ser alocado
    int R;                      // Bit de referência (utilizado no algoritmo NUR)
    int M;                      // Bit de modificação (utilizado no algoritmo NUR)
    int presente_na_memoria;    // Flag indicando se o processo está presente na memória
    int classe;                 // Classe do processo baseada nos bits R e M
} Processo;

// Estrutura que representa um bloco de memória
typedef struct {
    int inicio;                 // Endereço inicial do bloco na memória
    int tamanho;                // Tamanho do bloco de memória
    int livre;                  // Flag indicando se o bloco está livre (1) ou ocupado (0)
    Processo *processo;         // Ponteiro para o processo alocado neste bloco (NULL se livre)
} BlocoMemoria;

// Variável global para rastrear o tempo atual da simulação
int tempo_atual = 0;
...
void inicializaMemoria(BlocoMemoria memoria[], int tamanho_memoria, int *num_blocos_memoria) {
    ...
    memoria[0].livre = 1;                      // Inicialmente, todo o bloco está livre
...
void inserirProcessoNaFila(Processo fila[], int *qtd_processos, Processo processo) {
    ...
    (*qtd_processos)++;                // Incrementa a contagem de processos na fila
}

// Função para atualizar a classe do processo com base nos bits R e M
void atualizarClasse(Processo *processo) {
    processo->classe = (processo->R << 1) | processo->M; // Calcula a classe: (R * 2) + M
}

// Função para compactar a memória após a remoção de um processo
void compactarMemoria(BlocoMemoria memoria[], int *num_blocos_memoria, int tamanho_memoria) {
    int posicao_atual = 0;                                    // Posição atual para realocação
    BlocoMemoria memoria_compactada[MAX_PROCESSOS];          // Array temporário para memória compactada
    int num_blocos_compactados = 0;                           // Contador de blocos compactados
    ...
    for (int i = 0; i < *num_blocos_memoria; i++) {
        ...
        if (!memoria[i].livre && memoria[i].processo != NULL) {
            memoria_compactada[num_blocos_compactados] = memoria[i];      // Copia o bloco para o array compactado
            ...
            num_blocos_compactados++;                                       // Incrementa o contador
        }
    }
    ...
    // Atualiza a memória principal com os blocos compactados
...
// Função para remover um processo da memória e compactar a memória
void removerProcessoDaMemoria(BlocoMemoria memoria[], int indice, int *num_blocos_memoria, int tamanho_memoria) {
    ...
    memoria[indice].processo->presente_na_memoria = 0; // Atualiza a flag do processo
    memoria[indice].processo = NULL;                  // Remove a referência ao processo
...
void dividirBloco(BlocoMemoria memoria[], int *num_blocos_memoria, int indice, int tamanho_processo) {
    ...
    if (memoria[indice].tamanho > tamanho_processo) {
        ...
        (*num_blocos_memoria)++; // Incrementa o número de blocos de memória
...
int alocarProcessoFirstFit(BlocoMemoria memoria[], int *num_blocos_memoria, Processo *processo, int tamanho_memoria) {
    ...
    for (int i = 0; i < *num_blocos_memoria; i++) {
        ...
        if (memoria[i].livre && memoria[i].tamanho >= processo->tamanho) {
            ...
            processo->presente_na_memoria = 1;                          // Atualiza a flag do processo
...
Processo* selecionarProcessoParaRemover(BlocoMemoria memoria[], int num_blocos_memoria, int *indice_removido) {
    ...
    for (int classe = 0; classe <= 3; classe++) { // Percorre as classes em ordem crescente
